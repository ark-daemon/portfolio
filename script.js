...
(function () {
  ...
  /* ===== Hero ASCII portrait (text grid from photo) ===== */
  (function initHeroAscii() {
    var host = document.getElementById('hero-ascii');
    if (!host) return;

    var pre = host.querySelector('.hero-ascii-pre');
    var fallback = host.querySelector('.hero-photo-fallback');
    if (!pre) return;

    /* Dark → light density (matches mono face well) */
    var CHARS = '@#%*+=-:. ';
    var FOCUS_Y = 0.2; /* match object-position: center 20% */
    var img = new Image();
    var ready = false;
    var raf = 0;

    function drawCover(ctx, image, tw, th) {
      ...
      if (ir > cr) {
        ...
        /* image taller — crop top/bottom with face bias */
        sw = image.naturalWidth;
        sh = sw / cr;
        sx = 0;
        var maxSy = Math.max(0, image.naturalHeight - sh);
        /* place focus point (FOCUS_Y) near 20% of the destination box */
        sy = FOCUS_Y * image.naturalHeight - FOCUS_Y * sh;
    ...
    function measureCell(fontSize) {
      ...
      probe.style.cssText =
        ...
        'font-weight:' + (cs.fontWeight || '500') + ';' +
        'font-size:' + fontSize + 'px;line-height:1;letter-spacing:0;';
      document.body.appendChild(probe);
      var box = probe.getBoundingClientRect();
      var w = box.width || fontSize * 0.6;
      var h = box.height || fontSize;
    ...
    function render() {
      ...
      var fs = parseFloat(getComputedStyle(pre).fontSize) || 6;
      var cell = measureCell(fs);
      var cols = Math.max(8, Math.floor(cssW / cell.w));
      var rows = Math.max(8, Math.floor(cssH / cell.h));
      /* Cap density so mobile stays light (~12k cells max) */
      var maxCells = 14000;
      if (cols * rows > maxCells) {
        var scale = Math.sqrt(maxCells / (cols * rows));
      ...
      for (var y = 0; y < rows; y++) {
        ...
        for (var x = 0; x < cols; x++) {
          ...
          /* Slight contrast stretch so features read at low cell counts */
          lum = Math.max(0, Math.min(255, (lum - 16) * (255 / 220)));
          var idx = Math.round((lum / 255) * nChars);
          row += CHARS.charAt(idx);
        }
        lines[y] = row;
      }
      ...
      host.classList.add('is-ascii-ready');
    }
    ...
    img.onerror = function () {
      /* keep grayscale photo fallback */
      host.classList.remove('is-ascii-ready');
    };
    img.src = (fallback && fallback.getAttribute('src')) || 'hero-photo.png';
    ...
    /* Re-measure mono metrics after theme font paint settles */
    document.querySelectorAll('.theme-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
